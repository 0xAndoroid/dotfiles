...
```bash
agentcash fetch https://stableenrich.dev/api/exa/search -m POST -b '{
...
```bash
agentcash fetch https://stableenrich.dev/api/exa/find-similar -m POST -b '{
...
```bash
agentcash fetch https://stableenrich.dev/api/exa/contents -m POST -b '{
  "urls": [
...
```bash
agentcash fetch https://stableenrich.dev/api/exa/answer -m POST -b '{"query": "What is the population of Tokyo?"}'
```
...
```bash
agentcash fetch https://stableenrich.dev/api/firecrawl/scrape -m POST -b '{"url": "https://example.com/page-to-scrape"}'
```
...
```bash
agentcash fetch https://stableenrich.dev/api/firecrawl/search -m POST -b '{
...
```bash
agentcash fetch https://stableenrich.dev/api/exa/search -m POST -b '{"query": "AI agents in healthcare 2024", "numResults": 15}'
```

```bash
agentcash fetch https://stableenrich.dev/api/exa/find-similar -m POST -b '{"url": "https://best-article-found.com"}'
```

```bash
agentcash fetch https://stableenrich.dev/api/exa/contents -m POST -b '{"urls": ["url1", "url2", "url3"]}'
```
...
```bash
agentcash fetch https://stableenrich.dev/api/firecrawl/scrape -m POST -b '{"url": "https://blocked-site.com/article", "waitFor": 3000}'
```
