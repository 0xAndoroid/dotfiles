...
```bash
agentcash fetch https://stableupload.dev/api/upload -m POST -b '{"filename": "report.pdf", "contentType": "application/pdf", "tier": "10mb"}'
```
...
```bash
agentcash fetch https://stableupload.dev/api/uploads
```

## Get Upload Details

```bash
agentcash fetch https://stableupload.dev/api/download/k7gm3nqp2
```
...
- **Discovery endpoint**: `agentcash discover https://stableupload.dev` if you need to verify endpoints
...
- **Insufficient balance**: Show the deposit link from `agentcash wallet info`
