...
<Purpose>
Deep Interview implements Ouroboros-inspired Socratic questioning with mathematical ambiguity scoring. It replaces vague ideas with crystal-clear specifications by asking targeted questions that expose hidden assumptions, measuring clarity across weighted dimensions, and refusing to proceed until ambiguity drops below the resolved threshold for this run. The output is a crystallized spec file that can feed into any downstream planning or execution workflow.
</Purpose>
...
- User wants to explore options or brainstorm -- use a planning workflow instead
- User wants a quick fix or single change -- handle directly
- User says "just do it" or "skip the questions" -- respect their intent
- User already has a PRD or plan file -- proceed with that plan directly
</Do_Not_Use_When>

<Why_This_Exists>
AI can build anything. The hard part is knowing what to build. Single-pass spec expansion struggles with genuinely vague inputs because it asks "what do you want?" instead of "what are you assuming?" Deep Interview applies Socratic methodology to iteratively expose assumptions and mathematically gate readiness, ensuring the AI has genuine clarity before spending execution cycles.
...
3.5. **Load runtime settings**:
   ...
   - Resolve `deepInterview.ambiguityThreshold` into `<resolvedThreshold>`; if it is undefined, use `0.2`
...
4. **Initialize state** by writing to `.claude/state/deep-interview-state.json`:
...
Update interview state with the new round and scores by writing to `.claude/state/deep-interview-state.json`.
...
1. **Hand off the spec for direct execution**
   - Description: "The crystallized spec file is ready. Pass the spec file path to your preferred execution flow (planner, agent, or implementation pass)."
   - Action: Surface the spec file path and stop. The downstream caller is responsible for execution.

2. **Refine further**
   - Description: "Continue interviewing to improve clarity (current: {score}%)"
   - Action: Return to Phase 2 interview loop.

**IMPORTANT:** Deep Interview is a requirements skill, not an execution skill. Once the spec is written, hand off to whatever execution mechanism the caller prefers — direct implementation, a planning workflow, or another agent.

</Steps>

<Tool_Usage>
- Use `AskUserQuestion` for each interview question — provides clickable UI with contextual options
- Use the `Explore` subagent (haiku) for brownfield codebase exploration (run BEFORE asking user about codebase)
- Use opus model (temperature 0.1) for ambiguity scoring — consistency is critical
- Persist interview state to `.claude/state/deep-interview-state.json` via the `Write` tool
- Use `Write` tool to save the final spec to `.claude/specs/`
- Hand off the spec file path to the caller — never implement execution directly
...
- [ ] Spec file written to `.claude/specs/deep-interview-{slug}.md`
...
- [ ] Spec file path surfaced for the caller (deep-interview never executes directly)
...
If interrupted, run `/deep-interview` again. The skill reads state from `.claude/state/deep-interview-state.json` and resumes from the last completed round.
