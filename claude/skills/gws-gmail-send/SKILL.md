...
`+send` doesn't support `--from`. To send from a configured Gmail alias (e.g. `ee@andrew.ee`), construct a raw RFC 2822 message with the From header and use the raw API:

```bash
RAW=$(printf 'MIME-Version: 1.0\r\nFrom: Andrew Tretyakov <ee@andrew.ee>\r\nTo: recipient@example.com\r\nSubject: Hello\r\nContent-Type: text/plain; charset=utf-8\r\n\r\nBody text here' | base64 | tr '+/' '-_' | tr -d '=\n') && \
gws gmail users messages send --params '{"userId":"me"}' --json "{\"raw\":\"$RAW\"}"
```

The From address must be a verified "Send mail as" alias in Gmail settings.
