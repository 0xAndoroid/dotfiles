...
- [ ] Use `agentcash discover https://stableenrich.dev` to list all endpoints
- [ ] Use `agentcash check <endpoint-url>` to see expected parameters and pricing
- [ ] Call endpoint with `agentcash fetch`
- [ ] Parse and present results

```bash
agentcash fetch https://stableenrich.dev/api/apollo/people-enrich -m POST -b '{"email": "user@company.com"}'
```
...
```bash
agentcash fetch https://stableenrich.dev/api/apollo/people-enrich -m POST -b '{
...
```bash
agentcash fetch https://stableenrich.dev/api/apollo/org-enrich -m POST -b '{"domain": "stripe.com"}'
```
...
```bash
agentcash fetch https://stableenrich.dev/api/apollo/people-search -m POST -b '{
...
```bash
agentcash fetch https://stableenrich.dev/api/apollo/org-search -m POST -b '{
  "q_keywords": "fintech",
...
```bash
agentcash fetch https://stableenrich.dev/api/clado/linkedin-scrape -m POST -b '{"linkedin_url": "https://linkedin.com/in/johndoe"}'
```
...
```bash
agentcash fetch https://stableenrich.dev/api/clado/contacts-enrich -m POST -b '{
  "linkedin_url": "https://linkedin.com/in/johndoe",
...
```bash
agentcash fetch https://stableenrich.dev/api/apollo/people-enrich/bulk -m POST -b '{
  "people": [
...
```bash
agentcash fetch https://stableenrich.dev/api/apollo/org-enrich/bulk -m POST -b '{
  "organizations": [
    {"domain": "company1.com"},
    {"domain": "company2.com"}
  ]
...
```bash
agentcash fetch https://stableenrich.dev/api/hunter/email-verifier -m POST -b '{"email": "john@stripe.com"}'
```
...
```bash
agentcash fetch https://stableenrich.dev/api/influencer/enrich-by-email -m POST -b '{
...
```bash
agentcash fetch https://stableenrich.dev/api/influencer/enrich-by-social -m POST -b '{
  "platform": "instagram",
