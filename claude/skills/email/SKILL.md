...
Free endpoints use SIWX wallet authentication (handled automatically by `agentcash fetch`).
...
```bash
agentcash fetch https://stableemail.dev/api/send -m POST -b '{
...
```bash
agentcash fetch https://stableemail.dev/api/inbox/buy -m POST -b '{
  "username": "alice",
...
```bash
agentcash fetch https://stableemail.dev/api/inbox/send -m POST -b '{
  "username": "alice",
  "to": ["bob@example.com"],
  "subject": "Hello",
  "html": "<p>Hi Bob</p>",
...
```bash
agentcash fetch https://stableemail.dev/api/inbox/topup -m POST -b '{"username": "alice"}'
```
...
```bash
agentcash fetch https://stableemail.dev/api/inbox/cancel -m POST -b '{"username": "alice"}'
```
...
```bash
agentcash fetch https://stableemail.dev/api/inbox/update -m POST -b '{"username": "alice", "retainMessages": true}'
```

### List Messages

```bash
agentcash fetch https://stableemail.dev/api/inbox/messages -m POST -b '{"username": "alice", "limit": 20}'
```

### Read a Message

```bash
agentcash fetch https://stableemail.dev/api/inbox/messages/read -m POST -b '{"messageId": "msg_abc123"}'
```
...
```bash
agentcash fetch https://stableemail.dev/api/subdomain/buy -m POST -b '{"subdomain": "yourname"}'
```
...
```bash
agentcash fetch https://stableemail.dev/api/subdomain/send -m POST -b '{
...
```bash
agentcash fetch https://stableemail.dev/api/subdomain/inbox/create -m POST -b '{
  "subdomain": "yourname",
  "localPart": "support",
...
```bash
agentcash fetch https://stableemail.dev/api/subdomain/update -m POST -b '{"subdomain": "yourname", "catchAllForwardTo": "catch-all@company.com"}'
```
...
```bash
agentcash fetch https://stableemail.dev/api/subdomain/signers -m POST -b '{"action": "add", "subdomain": "yourname", "walletAddress": "0x..."}'
```
