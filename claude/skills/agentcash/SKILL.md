...
description: |
  ...
  Run `agentcash discover <origin>` to get endpoints, pricing, and usage instructions for any service below.
...
| Task | Command |
|------|---------|
| Check balance | `agentcash wallet info` |
| Redeem invite code | `agentcash wallet redeem <code>` |
...
```bash
agentcash discover <origin>
```

Example: `agentcash discover https://stableenrich.dev`
...
```bash
# POST
agentcash fetch <url> -m POST -b '{"key": "value"}'

# GET
agentcash fetch '<url>?param=value'
```
...
Run `agentcash discover <origin>` on any origin to see its full endpoint catalog.
...
- Use `agentcash check <url>` when unsure about request/response format.
